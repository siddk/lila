...
Core class definition for datasets and preprocessing, and other necessary startup steps to run before
training language-conditioned imitation learning models.
"""
import json
import logging
import os
...
overwatch = logging.getLogger("lila.preprocessing.imitation_dataset")
...
class SingleLanguageDemo(Dataset):
    ...
    def __init__(
        ...
        # Filter out "<GRIP>" actions -- User takes care of this for IL Models
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Augmentation (if noising!)
        if self.noise:
            state += self.noise_vec[idx]

        # Compute Next State as finite difference with (idx + window) in demonstration!
...
def get_imitation_dataset(
    ...
    overwatch.info("Preparing Imitation Learning Demonstration Dataset...")
